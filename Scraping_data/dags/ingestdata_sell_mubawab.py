...
def copy_csv_to_table_mubawab():
    try:
        ...
        cur.execute("""
            CREATE TEMP TABLE temp_sell_data_mubawab AS 
            SELECT * FROM Real_Estate_table_mubawab_vente LIMIT 0;
        """)

        # Copy CSV data to the temporary table
        csv_file_path = '/opt/airflow/dags/scraped_data_sell_mubawab.csv'
        with open(csv_file_path, "r") as file:
            cur.copy_expert(
                """
                COPY temp_sell_data_mubawab(Title, Real_estate_type, Ville, Secteur, Surface_totale, 
        ...
        cur.execute("""
            INSERT INTO Real_Estate_table_mubawab_vente (
            ...
            FROM temp_sell_data_mubawab temp
            WHERE NOT EXISTS (
                SELECT 1 FROM Real_Estate_table_mubawab_vente main
        ...
        with open(csv_file_path, "r") as infile, open(csv_file_path + ".tmp", "w", newline='') as outfile:
            ...
            for row in reader:
                ...
                cur.execute("""
                    SELECT 1 FROM Real_Estate_table_mubawab_vente
                    WHERE Title = %s
