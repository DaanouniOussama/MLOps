...
class Cleaning(Strategy):
    ...
    def handle_data(self, data : pd.DataFrame) -> pd.DataFrame:
        try:
            ...
            twenty_quantile = data.groupby('neighbourhood_')['price'].quantile(0.05).reset_index()
            eighty_quantile = data.groupby('neighbourhood_')['price'].quantile(0.95).reset_index()
            twenty_quantile.rename(columns={'price': '0.05_price'}, inplace=True)
            eighty_quantile.rename(columns={'price': '0.95_price'}, inplace=True)
            adding_2 = pd.merge(data , twenty_quantile, on='neighbourhood_', how='left')
            final_df = pd.merge(adding_2 , eighty_quantile, on='neighbourhood_', how='left')
